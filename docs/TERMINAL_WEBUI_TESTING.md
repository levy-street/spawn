...
- `web/tests/e2e/scrollback.spec.ts` covers history scrolling now that the terminal owns one buffer and the separate wheel-scrollback overlay is gone (`bb51ab7`). It drives wheel and touch scroll against the live terminal and asserts the overlay never opens; the slow-frame spec that guarded the old overlay's xterm.js Viewport NaN race went with the overlay.
...
- `scripts/smoke-local-browser-live.sh` starts the API server, web app, daemon, and a real browser. It verifies a live shell session, WebRTC terminal bytes, upload, and second-tab display control.
