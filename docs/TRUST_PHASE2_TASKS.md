...
| ID | Status | Scope | Depends on | Review/acceptance gate |
|----|--------|-------|------------|------------------------|
| P2-AGENT-01 | ACTIVE — IMPLEMENTED, REVIEW PENDING | Add versioned per-agent `spawn.ctl`; move history/snapshot plus resize/scroll/redraw/display ownership to it using bounded tmux/worker replay sources | GATE-02–05, QUAL-01 | Ordering/reconnect/size/error and multi-viewer viewport tests pass; tmux adoption seeds at most 10,000 configured-history lines, preserves an exact signal-free stream to the 12 MiB cap, and fails replay closed on overflow; worker replay rotates whole segments at its 8 MiB default and becomes unavailable after worker exit; both backends replay after `spawnd` restart/adoption; v2 server sees no history/snapshot/dimensions/deltas/event timing; reviewer notes that `0x01` still remains |
