...
| P2-HOST-03A | ACTIVE — IMPLEMENTED, REVIEW PENDING | Parallel interactive `tool.check`/`tool.install` on `spawn.host.ctl`: metadata-only REST discovery; endpoint-owned exact argv/no-shell execution; bounded processes/time/output/concurrency; cancellation/teardown and `outcome_unknown` reconciliation; browser result binding; legacy server/unattended route temporarily retained | P2-HOST-01, QUAL-03 | Adversarial daemon/browser/server tests cover malicious fields/results, bounds, cancel/timeout/close/reconnect/replacement, lost acknowledgement/no retry, concurrency, partial effect, zero-agent operation, and server/log/protocol inventories; compatibility retention and incomplete Phase 2 are explicit; independent review and mergeability pass |
