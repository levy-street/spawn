...
| P3-IDENTITY-02E | ACTIVE — REVIEWED, CHECKPOINT VALIDATED (`2e48b82`) | Require the approving browser tuple on daemon device-poll success, independently validate and fingerprint it after the host/token binding, and atomically merge at most 32 immutable daemon-local browser pins into protected credentials; expose redacted status and read-only lookup only | P3-IDENTITY-01B, P3-IDENTITY-02B, P3-IDENTITY-02C, P3-IDENTITY-02D | Independent review and the 02D integration checkpoint passed strict poll binding, legacy credential loading, deterministic idempotent merge/conflict/cap behavior, whole-record keyring/0600 generation recovery, cross-process locked base-revision CAS with stale-writer rejection, relogin/save-failure/logout semantics, exact real-login tuple persistence, status redaction, daemon/server/web/Chromium/build/guard gates, and mergeability. It is not merge-ready until the separately reviewed registration-canonicalization fix lands on master and is integrated/retested. Server revocation does not delete a local pin; explicit re-pair/local management remains future work; no live-signaling claim exists |
| P3-AUDIT-01 | ACTIVE — HARD GATE | Fresh adversarial audit of all five Phase 3 foundation workstreams: signed wire, host identity key, daemon-local browser pins, browser registration, and server/browser host pairing | P3-IDENTITY-01A, P3-IDENTITY-01B, P3-IDENTITY-02A, P3-IDENTITY-02B, P3-IDENTITY-02C, P3-IDENTITY-02D, P3-IDENTITY-02E | Independent auditors must obtain zero unresolved findings after explicit host/browser/nonce/session/scope/role/key substitution attacks, TOFU and first-contact analysis, all-ingress strict canonicalization review, and real Rust-to-WebCrypto plus WebCrypto-to-Rust exchanges. No foundation may be called trusted or done as a combined trust model before this gate passes |
| P3-IDENTITY-02 | PLANNED — LIVE SIGNING/TOFU GATE | Integrate signed agent/host signaling over the live WebSocket routes using the canonical tuple containing SDP, session ID, scope type/ID, protocol version, sender role, and intended peer key; consume the registered browser device identity and enforce peer-key discovery, TOFU continuity, and fingerprint UX | P3-IDENTITY-01A, P3-IDENTITY-01B, P3-IDENTITY-02A, P3-IDENTITY-02B, P3-IDENTITY-02C, P3-IDENTITY-02D, P3-IDENTITY-02E, P3-AUDIT-01 | Real live agent and host WebSocket/RTC exchanges must use the signed envelope on every ingress. With independently trusted/verifiable endpoint builds, fingerprint substitution plus cross-session, cross-agent, cross-host, cross-scope, protocol-version, role, and peer-key replay all make both endpoints abort loudly; unverified hosted JavaScript is explicitly not covered |
...
combined trust-model claim, and live signed WebSocket signaling/TOFU remains a
separate P3-IDENTITY-02 implementation and review gate.
...
6. **Parallel Phase 3 foundation (active):** 01A, 01B, 02A, 02B, offline 02C,
   and 02D are reviewed and integrated. The reviewed 02E daemon-local pin
   candidate has passed combined checkpoint validation. It awaits the reviewed
   registration-canonicalization master fix and a final integration retest.
   Active P3-AUDIT-01 requires a fresh zero-unresolved-finding five-workstream
   adversarial review. Peer
   discovery, TOFU, live P3-IDENTITY-02 WebSocket signaling, and every resulting
