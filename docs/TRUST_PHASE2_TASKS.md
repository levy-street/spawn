...
| P3-LIVE-F7 | ACTIVE — IMPLEMENTED, REVIEW PENDING | Require fresh daemon host-key proof over the exact device code, approval nonce, and strict host key before activating browser review or token issue; preserve the F8 claim/deletion fence | P3-IDENTITY-01B, P3-IDENTITY-02D | Independent review must pass Rust-produced/Python-verified fixed-width vectors, malformed/high-S/invalid-R and binding replay rejection, missing-proof fail-closed behavior, SQLite/PostgreSQL proof/delete/expiry races, pre-upgrade null proof migration, real daemon login and browser/daemon smokes, private-material isolation, and mergeability |
