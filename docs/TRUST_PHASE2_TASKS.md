...
| P2-HOST-03A | ACTIVE — IMPLEMENTED, REVIEW PENDING | Parallel interactive `tool.check`/`tool.install` on `spawn.host.ctl`: metadata-only REST discovery; endpoint-owned exact argv/no-shell execution; independently owned reap/pipe drain with permits and install claims retained through cleanup; bounded processes/time/output/concurrency; reachable cancellation and `outcome_unknown` check-before-retry reconciliation; canonical built-in agent-kind binding; legacy server/unattended route temporarily retained | P2-HOST-01, QUAL-03 | Adversarial daemon/browser/server tests cover malicious fields/results, bounds, abort/timeout/session-close/reconnect/replacement, sibling drain and no orphan/zombie/late publication, lost acknowledgement/no retry, concurrency, partial effect, zero-agent operation, and fail-closed server/web/daemon production inventories plus guard self-test; compatibility retention and incomplete Phase 2 are explicit; independent review and mergeability pass |
