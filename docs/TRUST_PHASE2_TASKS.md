...
| P2-HOST-03A | ACTIVE — IMPLEMENTED, REVIEW PENDING | Parallel interactive `tool.check`/`tool.install` on `spawn.host.ctl`: metadata-only REST discovery; endpoint-owned exact argv/no-shell execution; fail-closed Linux cgroup v2 lifecycle containment with pids-only delegation, a stable manager leaf and separate attempts subtree, verified per-attempt `pids.max`, inherited Landlock ABI 3 mutation policy, and architecture-reviewed seccomp credential firewall; stable-handle direct-child and subreaper-owned descendant reap/pipe drain with permits/install claims retained through absolute-deadline offloaded cleanup or a capped, globally admission-gating residual quarantine; generation-bound shared interactive/compatibility same-tool ambiguity gate; fresh exact-target REST reconciliation; canonical built-in agent-kind binding; legacy server/unattended route temporarily retained | P2-HOST-01, QUAL-03 | Adversarial daemon/browser/server tests cover malicious and contradictory fields/results, native-ABI/x32 seccomp dispatch, direct-parent and AF_UNIX user-manager-assisted cgroup escape after fork/exec, exit-zero closed-stdio `setsid` descendants, stalled freeze/inventory/populated/reap/removal phases with retained task ownership and post-kill zombie discovery, PID-reuse-safe direct-child quarantine, missing/mismatched/unlimited `pids.max`, exact-cap/cap-plus-one membership, bounded restart residue recovery, repeated pre-spawn cleanup/capacity, bounded residual capacity and repeated admission rejection, missing containment, latest/installed-version truncation uncertainty, abort/timeout/session-close/reconnect/replacement, sibling drain and no orphan/zombie/late publication, lost acknowledgement/no retry, overlapping check/effect generations, no-remount metadata drift, legacy auto-update inhibition, cross-path concurrency/ambiguity, partial effect, zero-agent operation, and fail-closed structural Python/TypeScript/Rust inventories including zero-baseline `attrgetter` references/reflection, stored constructor items, generic syscalls, dynamic-loader/escaped-FFI symbols, lexical shadowing, glob/type/extern aliases, all-production parsed token structure, and macro token trees; compatibility retention and incomplete Phase 2 are explicit; independent review and mergeability pass |
...
task. Do not add tmux repair TODOs to this ledger. Translate any old-backend bug
report into the equivalent `spawn-worker` behavior and schedule that
worker-only fix instead. Reversing the decision requires a new ADR and explicit
