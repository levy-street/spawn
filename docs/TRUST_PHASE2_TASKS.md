...
| P2-HOST-03A | ACTIVE — IMPLEMENTED, REVIEW PENDING | Parallel interactive `tool.check`/`tool.install` on `spawn.host.ctl`: metadata-only REST discovery; endpoint-owned exact argv/no-shell execution; fail-closed Linux cgroup v2 containment with subreaper-owned descendant reap/pipe drain and permits/install claims retained through `populated 0`; shared interactive/compatibility same-tool ambiguity gate; fresh exact-target REST reconciliation; canonical built-in agent-kind binding; legacy server/unattended route temporarily retained | P2-HOST-01, QUAL-03 | Adversarial daemon/browser/server tests cover malicious and contradictory fields/results, exit-zero closed-stdio `setsid` descendants and zero residue, missing containment, latest-probe uncertainty, bounds, abort/timeout/session-close/reconnect/replacement, sibling drain and no orphan/zombie/late publication, lost acknowledgement/no retry, no-remount metadata drift, legacy auto-update inhibition, cross-path concurrency/ambiguity, partial effect, zero-agent operation, and fail-closed Python/TypeScript/Rust AST inventories plus array-alias/unresolved-spawn guard self-test; compatibility retention and incomplete Phase 2 are explicit; independent review and mergeability pass |
