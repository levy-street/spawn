...
| P2-HOST-03B | BLOCKED — installation contained separately in #63 | Complete the tool cut: make the interactive endpoint path mandatory and relocate unattended executable policy/targets to the endpoint; legacy server-driven installation is already refused as security containment | P2-HOST-03A, P2-DATA-02, QUAL-03 | Restoring installation requires durable endpoint targets and a consumed tool-target effect generation; only conclusive version/package-manager proof resolves ambiguity; acknowledgement cannot authorize retry; server retains only disclosed policy/timestamps/content-free result and cannot persist detail |
...
| P3-LIVE-F2 | ACTIVE — BROWSER ENDPOINT CANDIDATE, REVIEW PENDING; DAEMON HALF OPEN | Make verified-signal types mandatory at RTC boundaries. The browser candidate freezes signed mode per agent/HostControl generation when supplied an exact epoch-scoped signing operation and active local Host pin, synchronously owns canonical snapshots of both pins and its route, verifies answers against those immutable expectations, compares the verified tuple with local route state, and gives WebRTC only `verified.transcript.sdp`. Daemon offer verification/live answer signing and the unconditional signed-only cutover remain open | P3-LIVE-F1, P3-LIVE-F4, P3-BROWSER-TRUST-SCOPE | Independent review must pass agent and HostControl raw-vs-verified-SDP substitution, stripped/malformed/signature/pin/peer/topology/replay/renegotiation failure, H1-to-H2 and browser-key mutation rejection plus fresh-generation rotation, retry and trust-invalidation teardown, fresh Rust-host-answer to live WebCrypto adapter and reverse verification, the self-tested grep-level exact caller guard, and current-master mergeability. F2 is not complete until daemon `handle_offer`/`handle_host_offer` consume verified offers and sign answers, all production callers supply independent local trust, and no raw caller remains |
...
| P3-LIVE-F4 | ACTIVE — BROWSER HOST-PIN CANDIDATE REVIEW PENDING | Establish reciprocal, independently persisted expected-peer pins. The browser candidate on `impl/p3-browser-host-pins` persists the locally derived/OOB-compared daemon key by exact account + canonical server origin, then binds bounded Host IDs only after an exact active key match; daemon selection and explicit OOB browser-pin activation remain separate required work. Never use a server-provided key as the verifier expectation | P3-IDENTITY-02D, P3-IDENTITY-02E | Candidate limits are IndexedDB schema/record version 1, 256 retained active-or-tombstone keys total, and 8 observed canonical Host IDs per key. Approval writes/reactivates the exact local key before the server POST. Host-detail resolution first requires exact route/response identity and may establish the bounded active binding; deletion then requires that pre-existing exact Host-ID-to-key binding, tombstones it before the same route-target DELETE, and never discovers or binds from deletion-time data. Partial failures are loud and retryable. Fake-IndexedDB and native Chromium coverage exercises strict keys/fingerprints, corruption/version/schema/cap, tab convergence, resolver refusal, split response/route IDs, key/fingerprint substitution, multiple active unbound pins, approval/deletion ordering, disappearance/reappearance, and exact explicit reapproval. Independent acceptance is pending. No RTC path consumes the resolver yet; this candidate alone makes no reciprocal daemon-pin, live-signing, TOFU-elimination, or L1 claim |
