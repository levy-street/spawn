...
| P3-DAEMON-TRUST-RELOAD | PLANNED — HARD BLOCKER | Keep live daemon authorization coherent after pin add/revoke or credential rotation. Replace `run()`'s one-time `StoredCreds` snapshot with revisioned whole-record reload/notification, or enforce a loud restart boundary; revalidate canonical server origin + Host ID and never mix token, host signing key, or pins across revisions | P3-IDENTITY-02E, P3-LIVE-F4 | While `run` is alive, adding a browser pin activates it promptly and revoking/removing one stops it; stale reconnect state cannot continue accepting the old pin. Token/key/pin reload is atomic and mismatched domain/revision fails closed. If restart is the design, admission is blocked until it occurs and UX/status make that boundary explicit |
