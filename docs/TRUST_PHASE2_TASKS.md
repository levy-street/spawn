...
| P3-IDENTITY-02E | ACTIVE — CORRECTION/RE-REVIEW | Consume the server-mediated approving-browser tuple into a daemon-local, bounded immutable pin foundation; bind every retained pin set to the canonical server origin plus returned Host ID; scope OS-keyring entries to the canonical config-directory identity; keep Unix's complete mode-0600 file as its sole commit point and its OS-keyring copy as optional redundancy | P3-IDENTITY-01B, P3-IDENTITY-02B, P3-IDENTITY-02C, P3-IDENTITY-02D | The integrated checkpoint and current correction candidate cover strict tuple validation, cross-config keyring isolation plus conflict-checked default-only legacy migration, schema/CAS/durability/fallback/relogin failure modes, exact two-login pin preservation, status/log redaction, and cross-runtime crypto. Commit results are typed at the authoritative boundary: ordinary Err guarantees no authoritative update, whereas native projection failure, complete-file post-rename directory-sync failure, and Unix keyring failure after file commit retain the committed whole generation in memory and report fixed redacted degraded/uncertain-durability warnings. Unix writes the file before the optional keyring; absent/empty/legacy first-save failure cannot publish N+1, and every reload selects an existing file regardless of stale/future keyring generation. Normal load/status may repair either optional copy once; bounded live-trust reload is read-only and quiet for both Unix keyring redundancy and native metadata projection. Independent re-review and current-master mergeability are still required. The poll source remains server-mediated: this row alone does not give an expected live peer key independent provenance, activate a pin, or establish TOFU/L1 |
...
| P3-LIVE-F1 | ACTIVE — IMPLEMENTED, REVIEW PENDING | Preserve the exact signed 12-field envelope in **both** offer and answer directions. The candidate carries one opaque `signed_envelope` string plus untrusted routing metadata through `ws/browser.py`, `ws/host.py`, `ws/daemon.py`, and Redis without reconstructing signed fields. Signed mode is immutable per RTC generation; raw/mixed answers then fail closed. Current endpoint callers remain explicitly legacy until F2/cutover | P3-AUDIT-01 | Independent review must confirm all three reconstruction/strip sites are closed, the calculated 512 KiB UTF-8 envelope bound fits one worst-case JSON nesting plus the 1,100 KiB WebSocket/1,200 KiB Redis limits, endpoint/relay/broker exact-limit/+1 checks pass, duplicate/missing/unknown/mixed fields fail closed, all signed fields and canonical key/signature substitutions survive byte-for-byte for endpoint rejection, and current-master mergeability passes. F2 verified-SDP consumption and L1 remain pending |
...
| P3-LIVE-F4 | ACTIVE — RECIPROCAL PIN CANDIDATES REVIEW PENDING | Establish reciprocal, independently persisted expected-peer pins. The browser candidate persists the locally derived/OOB-compared daemon key by exact account + canonical server origin, then binds bounded Host IDs only after an exact active key match. The daemon candidate locally derives a strict returned browser-key fingerprint and requires the exact full value independently copied from the browser's local display before first token/pin commit; only an exact durable device-ID/key pin in the same canonical origin + Host domain and OOB confirmation version 1 can authorize re-login without another first-contact comparison. Never use a server-provided key or fingerprint as the verifier expectation | P3-IDENTITY-02D, P3-IDENTITY-02E | Browser limits remain IndexedDB schema/record version 1, 256 retained active-or-tombstone keys total, and 8 observed canonical Host IDs per key. Daemon storage remains the capped 32-pin whole credential record with CAS/durability/keyring/config-dir scoping. Legacy 02E pins default to inert/unconfirmed conflict history and may only be atomically promoted for the same exact tuple after OOB confirmation; pre-authoritative save failure preserves legacy state, downgrade is impossible, and future marker versions fail closed. A native keyring-committed metadata failure is typed committed-degraded success: memory/reload retain the new token, generation, and v1 marker, status warns without secrets, and retry repairs or validates the projection. Unix commits its complete file before updating the optional keyring; file failure publishes no N+1 keyring state, while later keyring failure is typed redundancy-degraded. All reloads select the file without accepting stale/future keyring generations; normal load/status repairs the optional copy, while bounded live reload avoids repeated repair/log amplification. Complete-file post-rename directory-sync failure is likewise committed with explicitly uncertain crash durability; only pre-rename failure returns Err and leaves the target unchanged. Exact CLI or bounded interactive entry rejects trimmed/case/short/malformed/EOF and non-TTY first contact; tuple substitution, device/key/domain conflicts, capacity, and pre-commit save failures leave no new token/pin. Real login substitution proves byte-identical zero-write, and native Chromium retains the locally derived value across approval/success. Independent acceptance and current-master mergeability are pending. No RTC path consumes either resolver/pin yet, so the candidates make no live-signing, TOFU-elimination, or L1 claim |
| P3-BROWSER-TRUST-SCOPE | PLANNED — HARD BLOCKER | Scope every browser signer, host pin, warm terminal, and DataChannel capability to the canonical authenticated user UUID and active registration. `LiveTerminalProvider` must observe auth even though it is mounted above `AuthGate`; `HostControlClient` construction always requires explicit per-destination local trust material | P3-LIVE-F4, P3-LIVE-F5 | User becomes null, changes UUID, or registration is revoked/errors: all warm terminals close/evict and signer/pin capabilities are discarded. Account-switch/expired-session native tests pass. FileExplorer H1→H2 transfer resolves H2's distinct local signer/pin rather than inheriting H1; swapped pins fail. A bounded constructor/caller inventory prevents optional raw `new HostControlClient(destHostId)` trust |
| P3-DAEMON-TRUST-RELOAD | ACTIVE — IMPLEMENTED, REVIEW PENDING | Replace `run()`'s one-time credential snapshot with one revisioned whole-record authorization snapshot, immediate pre-connect and post-handshake/pre-registration rereads plus a bounded 500 ms reload poll, and an RTC trust-epoch fence. Revalidate canonical server origin + exact Host ID and never mix token, host signing key, or pins across revisions | P3-IDENTITY-02E, P3-LIVE-F4 | Independent review must prove add/revoke activation, atomic token/key/pin rotation, same-revision serialized substitution and rollback refusal, health-only projection transitions without reconnect/epoch teardown, read-only quiet live loads for both optional projections, stale-offer/reconnect fencing, missing/unavailable/corrupt backend failure, config/keyring isolation, local live-daemon reconnect behavior, bounded resource/shutdown behavior, no secret disclosure, and current-master mergeability |
