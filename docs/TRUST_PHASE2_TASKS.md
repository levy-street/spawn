...
| P3-IDENTITY-02E | ACTIVE — IMPLEMENTED, REVIEW PENDING | Require the approving browser tuple on daemon device-poll success, independently validate and fingerprint it after the host/token binding, and atomically merge at most 32 immutable daemon-local browser pins into protected credentials; expose redacted status and read-only lookup only | P3-IDENTITY-01B, P3-IDENTITY-02B, P3-IDENTITY-02C | Independent review proves strict poll binding, legacy credential loading, deterministic idempotent merge/conflict/cap behavior, whole-record keyring/0600 generation recovery, cross-process locked base-revision CAS with stale-writer rejection, relogin/save-failure/logout semantics, status redaction, and no live-signaling claim. Server revocation does not delete a local pin; explicit re-pair/local management remains future work |
| P3-IDENTITY-02 | PLANNED | Integrate signed agent/host signaling over the canonical tuple containing SDP, session ID, scope type/ID, protocol version, sender role, and intended peer key; consume the registered browser device identity and add TOFU pinning plus fingerprint UX | P3-IDENTITY-01A, P3-IDENTITY-01B, P3-IDENTITY-02A, P3-IDENTITY-02B, P3-IDENTITY-02C, P3-IDENTITY-02E | With independently trusted/verifiable endpoint builds, fingerprint substitution plus cross-session, cross-agent, cross-host, cross-scope, protocol-version, role, and peer-key replay all make both endpoints abort loudly for agent and host sessions; unverified hosted JavaScript is explicitly not covered |

The reviewed 01A, 01B, 02A, and offline 02C foundations plus bounded 02B and
02E may proceed in parallel with remaining Phase 2 cleanup because they do not
reopen a protected server content path. That overlap does not complete Phase 2
or establish a Phase 3 guarantee. Each task, and the later signed-signaling
