...
| P3-IDENTITY-02D | DONE — REVIEWED, MERGED (`7cc4ddc`) | Bind device-code approval to a fresh server nonce and exact active approving browser proof; transactionally create bounded immutable server Host/browser pins and return the exact browser tuple to poll | P3-IDENTITY-01B, P3-IDENTITY-02B | Independent review passed cross-runtime proof vectors, host/browser/nonce substitution, revoked-before-poll rejection, immutable and 32-pin bounds, SQLite/PostgreSQL approve/revoke/poll/cap races, native dual-fingerprint UX, response substitution, migrations, and mergeability. This is server/browser first contact only: daemon-local consumption is 02E and live signaling/TOFU remain unclaimed |
...
| P3-LIVE-F4 | PLANNED — HARD BLOCKER | Establish reciprocal, independently persisted expected-peer pins. Browser pins the locally derived/OOB-compared daemon key by account + canonical server origin + Host ID; daemon selects only its local browser-pin store and requires explicit OOB confirmation of the locally derived returned browser fingerprint before first persistence/activation. Never use a server-provided key as the verifier expectation | P3-IDENTITY-02D, P3-IDENTITY-02E | Real ceremony delivers the expected browser fingerprint out of band; a server-substituted browser tuple aborts before token/pin write. Browser host-pin mismatch/no pin is fatal; terminal pool/HostControl and daemon dispatch receive the local signing identity and local expected-peer pin explicitly. Host API disappearance/deletion never erases local trust or restores first-contact: explicit deletion writes a local revoked host/key tombstone with retryable server/local partial-failure recovery, and same Host ID reappearing with another key fails until explicit re-pair/rotation |
...
| P3-LIVE-F7 | PLANNED — HARD BLOCKER | Add daemon host-key proof-of-possession to pairing with a fresh non-replayable server challenge and a domain-separated transcript bound to the exact ceremony and key before approval/token issue | P3-IDENTITY-01B, P3-IDENTITY-02D | Missing, replayed, cross-code, cross-user, cross-host, wrong-key, and expired proofs fail before Host/pin/token creation; successful daemon proof and browser approval remain one bounded ceremony |
| P3-LIVE-F8 | ACTIVE — PARALLEL SERVER HARD BLOCKER | Retain a durable host key→owner claim after Host deletion and a deletion/device-code revocation fence so old ceremonies cannot recreate or transfer the identity | P3-IDENTITY-02D | Both invariants survive deletion, retries, restart, SQLite/PostgreSQL races, and stale device-code consumption; another owner can never claim the deleted key |
