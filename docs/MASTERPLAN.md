...
| B+C(Tier 2/3)+D+E D3 daemon | DONE (this commit) | Phase B: `release_key.rs` pinned key list (prod key `e65c013f`), manifest + `.sig` fetched from the daemon's own origin and Ed25519-verified over exact bytes before any sha256 is trusted, tree/hash cross-check, `release_counter` monotonic guard (`precondition/downgrade`, bypass only via `allow_downgrade`), build-time test overrides, `SPAWND_ALLOW_UNSIGNED_UPDATE=1` dev escape; corrupt probation marker no longer bricks startup; `worker_mismatch` is its own register key and a same-tree update repairs the pair; the two red `run::` tests made honest (ordinary close keeps peers — new sibling test; revoked key correctly refused — expectation fixed). Phase C: `--setup-token`/`SPAWN_SETUP_TOKEN` on `device/start`, `attended` suppresses the browser open with a 25 s fallback; the TUI kit (anstream/anstyle only, braille spinner on stderr, `[n/N]` steps, ≤12-line logo, NO_COLOR/non-TTY byte-identical), `state.json` heartbeat + SIGHUP reconnect, extended `status` (+`--json`, all instances), `doctor` (14 checks, `--json`, exit 1 — smoke-tested here), `reconnect`/`disconnect`/identity-keeping `logout`/`reset`, `exorcise` confirm, the §2.4 error catalogue, QR (`qrcode`, the only new crate), grouped help with goldens. Phase D: `registered.access_token` rotation persisted via the credential CAS; 1008 `token_*` → heartbeat `auth` + rate-limited log; `host.pin_adopted`/`host.pin_adopt_failed` frames. Phase E: `possess --new-account` + hint; status lists every instance. Report `IMPL-daemon-D3.md`. Independent re-verification: build + `clippy -D warnings` clean, `update::` 19, `login::` 19, `run::` 36, `creds::` 62, `ws::` 13, `proto::` 15, `session_ctl::` 10, `possess::` 5, `doctor::`/`tui::`/`cli::`/`state::` all green; release build OK; `spawnd help`/`status --json`/`doctor --json` smoke-run by hand. |
| A S2 server | DONE (this commit) | SPEC-connection S2 items 1–17 + the stale-presence addendum + `Host.last_disconnect_*` (alembic 0063): WS auth enforces `session_epoch` (1008); 25 s `ping` + `rtc.config` refresh/request on browser+host sockets; the close-code partition in `ws/close_codes.py` (4000 real supersession only, 4004 consistency, 4010 subscription lost, 4008 keepalive, 1012 shutdown); 60 s orphan grace + `live_bindings` rebind + `rtc.resume` + ICE restart on live bindings; `/ws/host` gets `protocol.required`/4003 and every socket rate-limited `error` frames; derived host status (online iff raw online AND `last_seen_at` ≤ 90 s) on every HTTP surface with opportunistic `stale` stamping; startup ICE URL validation + UDP-`turn:` warning; lock-free 10 s ownership cache on the relay hot path (a transient DB timeout drops one frame, never the socket); registration semaphore (32); candidate allowlist + 256-char status messages + 120 s pending TTL with `rtc.status expired`; pump readiness (4010 in 1 s) + eager Redis presence reclaim; `session_ice_policy`-gated session policy (the trap's guard test kept, its twin added); per-user/per-socket session-binding caps; one-per-process `?token=` deprecation warning. Report `IMPL-server-S2.md`. Independent re-verification: ruff clean, `alembic heads` = 0063, WS/hosts/turn/config/legion/relay suites 245 passed; the worker's full run 823 passed / 14 skipped. |
...
| D W4/M4 frontends | DONE (frontend half; server half rides S3) | Both frontends, one commit: Settings → "Sign out everywhere" (epoch bump; this session keeps a fresh token; 404 → "Not available on this server yet."); session renewal (web: cookie adopts itself; mobile: every response's `Set-Cookie` captured + half-life `POST /api/auth/session/renew` on foreground); host approving-devices list with `capacity {used,max}` warning at ≥28, per-pin "Not delivered", and the `host.pin_undelivered` alert → toast; Access panels sort by last seen with "Not seen since {date}" beyond 60 d (mobile gains last-seen parity); "Remove this account from this {browser|phone}" wiping only that account's identity/approvals/peer keys/root knowledge while revocation tombstones and the trust-revision floor survive, with tombstones counted apart from the active caps; honest "Clear history" copy. Reports `IMPL-web-D.md` (its "Notes" section misquotes a few strings — the code matches the contract and mobile word for word), `IMPL-mobile-D.md`. Independent re-verification: web lint/tsc clean, bun 1106/0, Playwright `settings-hygiene.spec.ts` + `setup-claims.spec.ts` 8/8; mobile `npm run ci` 233 suites / 1608 tests green. |
| F S4 server | DONE (this commit) | `/api/release.desktop` `{version, tree, platforms}` (null when the checkout is dirty, the dir/config is missing, or the identity is unknown; `SPAWN_DESKTOP_VERSION`/`SPAWN_DESKTOP_TREE` overrides like the other identities) and the desktop OAuth redirect `spawn://oauth/callback` in the default native allow-list beside mobile's. Report `IMPL-server-S4.md`. Independent re-verification: ruff clean, `alembic heads` = 0064, release/auth_providers/oauth suites 32 passed. |
| T (Part 8) T2 harness | DONE (this commit) | `scripts/test-update-e2e.sh` (two HEAD builds with tree/counter/key overrides, throwaway-signed manifest, local server with `SPAWN_PREBUILT_DIR`, scripted ceremony; asserts auto-update → same PID exec → tree-B re-register, `.prev` gone only after register, worker/session survive adoption, same-tree no-op, manual 202 then 429, disabled/unwritable preconditions, downgrade refused unless `allow_downgrade`), `scripts/test-update-faults.sh` + stdlib `scripts/fault-proxy.py` (truncate/flip/trickle/bad `--version`/manifest tamper/bad signature → the right stage, old pair intact), `scripts/test-update-probation.sh` (bad binary passes `--version`, crash-loops, reverts to `.prev`, host shows `health`, no re-push for two keepalive windows), `scripts/test-version-skew.sh` (the four-cell ritual, gated on `SPAWN_OLD_REF`), `scripts/chaos-drills.sh` (SIGSTOP/SIGCONT → offline within 90 s → back with zero supersessions; uvicorn SIGKILL → daemon keeps peers, re-registers; pfctl/NLC cells MANUAL behind `SPAWN_ALLOW_SUDO=1`), all wired into `test-all.sh`; `docs/RELEASE.md` "Proving the updater"; `web/tests/e2e/version-skew.spec.ts` (soft-mismatch cell passes; the 4003 cell is `fixme` — its mocked alerts socket is not reachable through a separately spawned old-web dev server). Report `IMPL-scripts-T2.md`. Independent re-run here: all self-tests ok; e2e 76 s PASS, faults 94 s PASS, probation 81 s PASS. Honest SKIPs: PTY-bytes-across-exec proof and the `rtc.status rebound` observation need a reusable non-browser RTC observer; the `hang → download` cell needs a test-only download-timeout hook in the daemon. |
