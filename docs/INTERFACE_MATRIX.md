...
| Restart a session | sidebar, pane/full-session actions, the agent's own "Update installed · Restart to update" notice | `POST /api/sessions/{id}/restart` (a fresh shell) or nothing (the agent relaunched in the shell it has) | `sessions.restart`; the client types the agent's resume command (`claude --resume <agent_session_id>`, or `--continue` when none was recorded) | `session.restart` starts a login shell in the stored `cwd`; the resume command is client-typed input like any launch |
