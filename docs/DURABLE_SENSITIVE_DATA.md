...
The durable-data guard checks these exact machine-readable rows and a few
literal design/status markers. It does not render Markdown, inventory prose, or
interpret English semantics. Prose changes require ordinary human review; a
passing source guard is not approval of this design or evidence of runtime
behavior. The permanent boundary is documented in `GUARD_POLICY.md`.
...
| `guard_policy` | `literal_machine_surfaces_only` |
| `english_semantic_guarding` | `forbidden` |
...
control plane can actively substitute peers until P3-IDENTITY-02 signed
...
- **P3-IDENTITY-01A/01B** build the independently gated crypto and host-pairing
  foundations; **P3-IDENTITY-02** closes malicious signaling substitution for
  trusted or independently verifiable endpoint builds. None is a reason to put
  keys on the server in the meantime.
