"""Migration 0062 (workspace home, second pass) adopts a live session's folder.
...
new window should open, forever. 0062 walks the tiles in the same order but
...
Pinned to revision 0062 rather than head so later migrations reshaping the
...
def test_0062_adopts_the_first_surviving_session_and_touches_nothing_else(tmp_path: Path):
