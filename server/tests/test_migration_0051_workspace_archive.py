...
byte unchanged. 0051 added `archived_at` and `archived_shape`; 0053 dropped
...
def test_0051_adds_nullable_archive_columns_without_touching_existing_rows(tmp_path: Path):
    db_path = tmp_path / "spawn-archive.db"
    env = _migration_env(f"sqlite+aiosqlite:///{db_path}")
    sync_url = f"sqlite:///{db_path}"

    _alembic(["upgrade", "0050"], env=env)
    ...
    try:
        with engine.begin() as conn:
            ...
            # 0053 took the snapshot column away again: an archived workspace
...
def test_0051_downgrade_drops_the_columns_and_keeps_the_workspaces(tmp_path: Path):
    ...
    _alembic(["downgrade", "0050"], env=env)
