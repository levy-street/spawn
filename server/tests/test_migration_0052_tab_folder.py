"""Migration 0052 (per-tab host/folder) backfills without rewriting the rest.
...
Pinned to revision 0052 rather than head: later migrations reshape the
...
def test_0052_backfills_tab_folders_from_the_first_session(tmp_path: Path):
    db_path = tmp_path / "spawn-tab-folder.db"
    env = _migration_env(f"sqlite+aiosqlite:///{db_path}")
    sync_url = f"sqlite:///{db_path}"

    _alembic(["upgrade", "0051"], env=env)
...
def test_0052_downgrade_strips_the_pair_and_keeps_everything_else(tmp_path: Path):
    ...
    _alembic(["upgrade", "0052"], env=env)
    _alembic(["downgrade", "0051"], env=env)
    engine = create_engine(sync_url, future=True)
    try:
        with engine.begin() as conn:
            # Back to the pre-0052 shape: same tabs, same tiles, no folder keys.
