"""Migration 0070 (billing) adds two tables and a column, and nothing else.
...
Pinned to revision 0070 rather than head for the same reason the neighbouring
...
def _upgraded(tmp_path: Path, name: str):
    """Seed at 0069, then upgrade to 0070. Returns the env and the sync URL."""
    db_path = tmp_path / name
    env = _migration_env(f"sqlite+aiosqlite:///{db_path}")
    sync_url = f"sqlite:///{db_path}"

    _alembic(["upgrade", "0069"], env=env)
    ...
    _alembic(["upgrade", "0070"], env=env)
    return env, sync_url


def test_0070_leaves_every_existing_account_without_an_override(tmp_path: Path):
...
def test_0070_holds_a_subscription_and_defaults_a_bare_row_to_free(tmp_path: Path):
...
def test_0070_refuses_a_subscription_the_server_could_not_mean(
...
def test_0070_allows_unlimited_and_a_subscription_that_does_not_exist_yet(tmp_path: Path):
...
def test_0070_keeps_one_subscription_per_account_and_per_stripe_object(
    tmp_path: Path, overrides: dict
):
...
def test_0070_rejects_a_redelivered_event_id(tmp_path: Path):
...
def test_0070_downgrade_removes_everything_it_added(tmp_path: Path):
    ...
    _alembic(["downgrade", "0069"], env=env)
