"""Migration 0054 (workspace icon) adds the pair without disturbing a row.
...
Pinned to revision 0054 rather than head for the same reason the neighbouring
...
def test_0054_leaves_every_existing_row_unlooked(tmp_path: Path):
    db_path = tmp_path / "spawn-workspace-icon.db"
    env = _migration_env(f"sqlite+aiosqlite:///{db_path}")
    sync_url = f"sqlite:///{db_path}"

    _alembic(["upgrade", "0053"], env=env)
...
def test_0054_downgrade_drops_the_pair_from_both_tables(tmp_path: Path):
    ...
    _alembic(["downgrade", "0053"], env=env)
    engine = create_engine(sync_url, future=True)
    try:
