"""Migration 0037 (12x12 grid -> 24x24) is safe whatever order things deploy in.

Scaling coordinates is not idempotent, and a doubled layout is indistinguishable
from an un-doubled one by inspection — so 0037 keys off the per-grid `version`
...
def test_0037_lifts_v2_once_and_leaves_v3_alone(tmp_path: Path):
    db_path = tmp_path / "spawn-grid-24.db"
    env = _migration_env(f"sqlite+aiosqlite:///{db_path}")
    sync_url = f"sqlite:///{db_path}"

    _alembic(["upgrade", "0036"], env=env)
...
def test_0037_downgrade_returns_the_old_space_and_is_also_idempotent(tmp_path: Path):
    ...
    _alembic(["upgrade", "head"], env=env)
    _alembic(["downgrade", "0036"], env=env)
