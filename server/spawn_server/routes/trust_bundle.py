...
@router.post("/device-approvals/{request_id}/deny", response_model=schemas.DeviceApprovalRequestOut)
async def deny_device_approval(
    ...
    if row.status == "pending":
        ...
        await session.commit()
        await publish_trust_event(user.id, approval_resolved_payload(row.id, device.id, "denied"))
