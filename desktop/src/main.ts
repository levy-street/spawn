  ...
  if (serverSupported === false) {
    return sheet(
      ...
      stacked(
        ...
        `<div class="stack"><div class="inset"><p class="muted">Signing in here needs an endpoint this server does not have yet, and ${OS_COPY.thisComputer} could not be possessed by it. Update the server to the current release, or point SPAWN D at another one.</p></div>${errorLine()}<div class="actions"><button class="btn btn-primary" data-action="choose-server">Choose another server</button><button class="btn btn-ghost" data-action="retry-config">Try again</button></div></div>`,
      ),
    );
  }
  ...
  if (refused) {
    ...
    action = `
      <div class="approve" data-testid="possess-approve-screen">
        ...
        <p>SPAWN D verified ${OS_COPY.thisComputer}’s identity against the link its daemon printed. Approving grants all your devices access to it.</p>
    ...
    action = `<button class="btn btn-primary btn-block" data-action="possess" ${busy ? "disabled" : ""}>${busy ? "Starting…" : `Possess ${OS_COPY.thisComputer}`}</button>`;
  ...
  const body = `
    <div class="stack">
      <div class="inset">
        ...
        <p class="muted">SPAWN D opens in a moment. It stays in your ${OS_COPY.tray}; the daemon keeps running on its own.</p>
      </div>
      ${errorLine()}
