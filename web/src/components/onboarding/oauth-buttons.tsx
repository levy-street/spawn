 ...
 *
 * Not decoration: Apple's guidelines require its mark to accompany "Sign in
 * with Apple", and Google's branding rules say the same for the G. Apple's is
 * monochrome by rule and takes the button's ink; the rest are drawn at brand
 * colours, which hold on either theme.
 */
function ProviderMark({ provider }: { provider: AuthProvider["id"] }) {
  if (provider === "apple") {
    return (
...
export function OAuthButtons({
  ...
  returnTo: string;
  /**
   * Carried to the provider and back so a closed deployment can admit the
   * account at the callback. Without it, a first-time provider sign-in on an
   * invite-only server is refused — the gate applies wherever accounts are
   * created, not just to the email form.
   */
  invite?: string | null;
  loading?: boolean;
}) {
