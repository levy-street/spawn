...
1. **Link a device once — check one number — and it can reach every machine.**
...
| **machine** | noun | host, daemon, anchor set A(h) |
...
- **machine** — what your agents run on: a server, a laptop, a VM. Not "host"
  (infrastructure jargon), not "computer" (sounds like a desktop). "Your devices reach
  your machines" parses on first read and pairs with the verb below.
...
- **possess** — attaching a machine. The one deliberate exception to the everyday-words
  rule: it is the product's own verb, the terminal literally prints `spawnd possess`, and
  the screen matching the terminal beats generic clarity. Still a different verb than
  *link* on purpose: machines enter from their own terminal, devices from another
  device's screen.
...
3. **Machines** — every possessed machine, same row grammar ("Possessed by MacBook Pro" — provenance never collides with the Online/Offline presence pill, which is presence, not trust).
...
There is no second destination, no per-machine trust page, no key inspector.

## Screens and states

### 1. Devices (roster) — `DevicesScreen`
- **default** — recovery on, devices + machines listed, history preview.
- **recovery off** — amber strip replaces the quiet one; everything else identical.
- Device rows are not stateful beyond the "This device" tint; machine rows dim when offline (presence, not trust).
...
- **compare · enter** — a six-digit field; wrong entry says "That's not it — N tries
  ...
  see a number"*.
- **compare · fingerprint** — older machines have no number; the same frame shows
...
- new device: **waiting** ("Confirm from a device you already use — or use your recovery
  passkey") → NumberCheck → **done** ("Linked. Every machine is ready.")
...
### 4. Possess a machine — `PossessMachine`
- **instruction** — "Run this on the machine: `spawnd possess`" (one command, one line).
...
- **confirm** — "Remove iPhone? It loses access to every machine — instantly and
  permanently. To use it again, you'd link it as a new device." *(That sentence is P3 +
  R10 + R1, in one breath.)*
- **orphan warning (R5)** — adds an amber block naming the machines at risk. Recovery
  ...
  (matches the shipped warn-not-refuse). Recovery on **and the machine online**: "You'll
  confirm with your passkey so it stays reachable" — the passkey touch heals the machine
  onto recovery before the removal lands. An **offline** machine can't be healed in time,
...
- **First device + first machine.** Sign up on the device (no ceremony — there is nothing
  to link *to*; the roster shows "First device"). Prompt to turn on recovery. Connect the
  first machine: run one command, type one number. Total human cost: one number.
- **Additional device, passkey.** Sign in → passkey → done. Zero screens. (Heal endorses
  it off the root; the bundle delivers the machines.)
...
- **Remove a device.** Overflow → Remove → confirm. If it would orphan a machine, the
...
|---|---|---|
| Hex key fingerprints to eyeball | Nothing on modern endpoints; the same check-frame shows a full fingerprint only for older machines | The committed SAS (A5) is *stronger* than fingerprint eyeballing; where a legacy endpoint can't do it, the protocol's rollout rule mandates the *full-entropy* compare — never a weaker short code — and the UI renders exactly that |
...
| Per-machine trust matrices (which device may reach which host) | Nothing | P1: every device reaches every machine. A matrix would be an N×M grid of identical checkmarks |
...
- The ceremonies (`NumberCheck`, `PossessMachine`, `LinkNewDevice`) keep their
...
- **First device + first machine flow** and all copy use the possess vocabulary end to end —
...
- **A5 number match** — kept in both ceremonies as ENTRY, not tap-to-approve (the
  ...
  tries are terminal, no override. Legacy machines get the full-fingerprint compare.
- **P3/R1/R10 revocation** — instant ("instantly"), account-wide ("every machine"),
...
- **R5 orphan hazard** — the remove dialog names the machines that would be orphaned and
