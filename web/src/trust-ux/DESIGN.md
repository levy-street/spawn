...
1. **Link a device once — check one number — and it can reach every computer.**
...
| **computer** | noun | host, daemon, anchor set A(h) |
...
| **connect** | verb | possess: host anchor ceremony |
...
- **computer** — the machine your agents run on. Not "host" (infrastructure jargon), not
  "daemon". "Your devices reach your computers" parses on first read.
...
- **connect** — attaching a computer. A different verb than *link* on purpose: computers
  enter from their own terminal, devices enter from another device's screen.
...
One destination: **Devices** (a settings section). It contains, top to bottom:
...
3. **Computers** — every connected computer, same row grammar ("Set up by MacBook Pro" — *connect* stays the ceremony verb; rows use *set up by* so provenance never collides with the Online/Offline presence pill).
4. **History** — the trust log (R4), newest first, plain sentences. Last three inline;
   "Everything" expands.

Flows (link, connect, remove, recovery) are entered from this screen or arrive as prompts.
There is no second destination, no per-computer trust page, no key inspector.

## Screens and states

### 1. Devices (roster) — `DevicesScreen`
- **default** — recovery on, devices + computers listed, history preview.
- **recovery off** — amber strip replaces the quiet one; everything else identical.
- Device rows are not stateful beyond the "This device" tint; computer rows dim when offline (presence, not trust).
...
- **compare · enter** — a six-digit field; wrong entry says "That's not it — N tries
  ...
  see a number"*.
- **compare · fingerprint** — older computers have no number; the same frame shows
...
- new device: **waiting** ("Confirm from a device you already use — or use your recovery
  passkey") → NumberCheck → **done** ("Linked. Every computer is ready.")
...
### 4. Connect a computer — `ConnectComputer`
- **instruction** — "On that computer, run: `spawnd possess`" (one command, one line).
- → NumberCheck (terminal prints the same number) → **done** ("mac-studio is connected.
  All your devices can reach it.")

### 5. Remove a device — `RemoveDeviceDialog`
- **confirm** — "Remove iPhone? It loses access to every computer — instantly and
  permanently. To use it again, you'd link it as a new device." *(That sentence is P3 +
  R10 + R1, in one breath.)*
- **orphan warning (R5)** — adds an amber block naming the computers at risk. Recovery
  ...
  (matches the shipped warn-not-refuse). Recovery on **and the computer online**: "You'll
  confirm with your passkey so it stays reachable" — the passkey touch heals the computer
  onto recovery before the removal lands. An **offline** computer can't be healed in time,
...
### 6. Recovery — the strip inside `DevicesScreen`, plus `TurnOnRecoveryDialog` and `ResetRecoveryDialog`
...
- Plain sentences, newest first: "MacBook Pro linked iPhone", "Recovery restored Pixel 9",
  "iPad removed by MacBook Pro — access ended everywhere", "mac-studio connected".
  Every live endorsement and every removal is one visible line (R4). No filters, no tabs.

## Flows

- **First device + first computer.** Sign up on the device (no ceremony — there is nothing
  to link *to*; the roster shows "First device"). Prompt to turn on recovery. Connect the
  first computer: run one command, compare one number. Total human cost: one number.
- **Additional device, passkey.** Sign in → passkey → done. Zero screens. (Heal endorses
  it off the root; the bundle delivers the computers.)
...
- **Remove a device.** Overflow → Remove → confirm. If it would orphan a computer, the
...
|---|---|---|
| Hex key fingerprints to eyeball | Nothing on modern endpoints; the same check-frame shows a full fingerprint only for older computers | The committed SAS (A5) is *stronger* than fingerprint eyeballing; where a legacy endpoint can't do it, the protocol's rollout rule mandates the *full-entropy* compare — never a weaker short code — and the UI renders exactly that |
...
| Per-computer trust matrices (which device may reach which host) | Nothing | P1: every device reaches every computer. A matrix would be an N×M grid of identical checkmarks |
...
- **The overflow menu earns its hop**: it holds Remove today and rename next; a bare
...
- **A5 number match** — kept in both ceremonies as ENTRY, not tap-to-approve (the
  ...
  tries are terminal, no override. Legacy computers get the full-fingerprint compare.
- **P3/R1/R10 revocation** — instant ("instantly"), account-wide ("every computer"),
...
- **R5 orphan hazard** — the remove dialog names the computers that would be orphaned and
