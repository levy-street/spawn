...
export const PHONE_SSH: ArticleEntry[] = [
  {
    ...
    kind: "guide",
    hub: { name: "On your phone", href: "/coding-agents-on-your-phone" },
    ...
    body: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "The sessions are different too. Each one is owned by a worker process on the host, so it survives the closed tab, the dropped connection, the laptop lid, and a daemon restart, scrollback intact — there is no tmux to remember. And what the phone opens onto is the standing work: one workspace per project, a grid of sessions across hosts, an attention cue when an agent is waiting on a yes, and a notification on the phone so you don’t have to keep checking. Your browser talks to each daemon peer-to-peer, end-to-end encrypted; the server that introduces them never sees session content. It is open source, MIT/Apache-2.0. The row-by-row case is at [spawnd vs mobile SSH apps](/spawnd-vs-mobile-ssh-apps), and [Claude Code on your phone](/claude-code-on-your-phone) shows what the grid is for.",
        ],
      },
      {
        kind: "capture",
        caption:
          "What the phone opens onto instead of a prompt: the grid of standing sessions, each on the host that owns it, the one waiting on you marked.",
      },
    ],
    ...
    related: [
      {
        ...
        blurb: "Termius and Blink polish the window; spawnd removes the client",
        href: "/spawnd-vs-mobile-ssh-apps",
      },
      {
        title: "Claude Code on your phone",
        blurb: "what a phone terminal is for in the agent era",
    ...
    body: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "Sessions live on the host — a worker process owns each one’s PTY — so closing the keyboard case, losing the café Wi‑Fi, or restarting the daemon costs nothing, and the scrollback is intact when you come back. What you come back to is a workspace per project with a grid of sessions across hosts, an attention cue on the one where an agent is waiting on a yes, and a notification when it happened. Your browser talks to each daemon peer-to-peer, end-to-end encrypted; the server that introduces them never sees session content. It is open source, MIT/Apache-2.0. [Code on an iPad](/code-on-an-ipad) shows the whole thing on the tablet, and [spawnd vs mobile SSH apps](/spawnd-vs-mobile-ssh-apps) is the case for keeping Blink beside it.",
        ],
      },
      {
        kind: "capture",
        caption:
          "The iPad as console: a workspace per project, sessions from three machines in one grid, the one that is waiting on you marked — with nothing installed but a browser.",
      },
    ],
    ...
    related: [
      {
        ...
        blurb: "the whole console on the tablet, in real captures",
        href: "/code-on-an-ipad",
      },
      {
        title: "spawnd vs mobile SSH apps",
        blurb: "why the tablet may not need an SSH client at all",
        href: "/spawnd-vs-mobile-ssh-apps",
      },
      {
    ...
    body: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "The battery manager stops mattering, because the session was never on the phone. Each one is owned by a worker process on the host, so it survives the closed tab, the dropped connection, and a daemon restart, scrollback intact, with no tmux to remember. What the phone opens onto is one workspace per project, a grid of sessions across hosts, an attention cue when an agent is waiting on a yes, and a notification so you don’t have to keep checking. Your browser talks to each daemon peer-to-peer, end-to-end encrypted; the server that introduces them never sees session content. It is open source, MIT/Apache-2.0. [Coding agents on your phone](/coding-agents-on-your-phone) is what that grid is for, and [spawnd vs mobile SSH apps](/spawnd-vs-mobile-ssh-apps) is the row-by-row case.",
        ],
      },
      {
        kind: "capture",
        caption:
          "The phone as console: standing sessions across hosts in one grid, the one waiting on you marked — nothing installed, nothing for the battery manager to kill.",
      },
    ],
    start: "One line on the host, nothing on the phone.",
    faq: [
    ...
    related: [
      {
        ...
        blurb: "the hub: what a phone terminal is for now",
    ...
    body: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "spawnd inverts it. One daemon on each macOS or Linux host you own; the daemon dials out, so nothing on the host listens — no open port, no VPN. Any browser is the console, so the Windows machine contributes a browser tab and nothing else: no client to install, no key on the device, one approval against a short code, and a one-click revocation every host honors when the laptop goes back. Sessions live on the host with a worker process owning each PTY, so they outlive the closed tab and the dropped connection, scrollback intact, in a workspace per project with a grid of sessions across hosts. Your browser talks to each daemon peer-to-peer, end-to-end encrypted; the server that introduces them never sees session content. It is open source, MIT/Apache-2.0. To be exact about the boundary: the daemon runs on macOS and Linux, and the Windows machine is the browser side — [spawnd vs SSH + tmux](/spawnd-vs-ssh-and-tmux) has the row-by-row case.",
        ],
      },
      {
        kind: "capture",
        caption:
          "From any browser, including the one on a locked-down Windows laptop: the standing sessions on the machines you own, one grid, nothing installed.",
      },
    ],
    ...
    related: [
      {
        ...
        blurb: "the plumbing every client above inherits, examined",
        href: "/spawnd-vs-ssh-and-tmux",
      },
      {
        title: "Remote access without open ports",
        blurb: "the outbound-only model behind the browser answer",
