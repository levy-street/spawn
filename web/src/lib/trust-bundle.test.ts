...
describe("trust bundle", () => {
  test("round-trips the hosts an operator has verified", async () => {
    ...
    const sealed = await sealTrustBundle(key, ACCOUNT, hosts);
  ...
  test("a different passkey secret cannot open the bundle", async () => {
    const sealed = await sealTrustBundle(await deriveTrustBundleKey(prf(1), ACCOUNT), ACCOUNT, [
      await host(),
    ]);
  ...
  test("the same secret under another account derives a different key", async () => {
    ...
    const sealed = await sealTrustBundle(await deriveTrustBundleKey(prf(3), ACCOUNT), ACCOUNT, [
      await host(),
    ]);
  ...
  test("a bundle cannot be replayed into another account", async () => {
    ...
    const sealed = await sealTrustBundle(key, ACCOUNT, [await host()]);
  ...
  test("a host entry whose fingerprint disagrees with its key is refused", async () => {
    ...
    await expect(sealTrustBundle(key, ACCOUNT, [lying])).rejects.toThrow(TrustBundleError);
  });
  ...
  test("a non-Ed25519 host key is refused", async () => {
    ...
    await expect(sealTrustBundle(key, ACCOUNT, [bogus])).rejects.toThrow(TrustBundleError);
  });

  test("duplicate host keys are refused", async () => {
    const key = await deriveTrustBundleKey(prf(8), ACCOUNT);
    const only = await host();
    await expect(sealTrustBundle(key, ACCOUNT, [only, only])).rejects.toThrow(TrustBundleError);
  });
  ...
  test("sealing is deterministic in content but not in ciphertext", async () => {
    ...
    const first = await sealTrustBundle(key, ACCOUNT, hosts);
    const second = await sealTrustBundle(key, ACCOUNT, hosts);
  ...
  test("host order does not affect the opened bundle", async () => {
    ...
    const forward = await openTrustBundle(
      key,
      ACCOUNT,
      await sealTrustBundle(key, ACCOUNT, [a, b]),
    );
    const reverse = await openTrustBundle(
      key,
      ACCOUNT,
      await sealTrustBundle(key, ACCOUNT, [b, a]),
    );
