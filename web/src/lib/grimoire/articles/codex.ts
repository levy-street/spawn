...
export const CODEX: ArticleEntry[] = [
  {
    ...
    related: [
      ...
      },
      {
        ...
        blurb: "Ten terminal agents, grouped by who they’re for, every fact checked.",
...
/** The cluster's pillar hub, racked into the catalogue by articles/index.ts. */
    ...
    essay: [
      ...
      },
      {
        ...
        paragraphs: [
          ...
          "spawnd is built for the other half. One daemon on each host you own dials out, so nothing listens on the host — no open ports, no VPN. Each session’s PTY is owned by a worker process on the host, so a Codex run survives the closed tab, the dropped connection, the laptop lid, and a restart of the daemon itself, scrollback intact. Any browser is the console, and on a phone it installs to the home screen as a web app; a new device is approved once against a short code, and revoking it is one click every host honors. Codex is a built-in shortcut — the button types `codex` into a real login shell on the host, so your sign-in, your `config.toml`, and your sandbox settings apply exactly as at the keyboard; spawnd holds no provider credentials and adds no API-key markup. Your browser talks to each daemon peer-to-peer, end-to-end encrypted; the server that introduces them never sees session content. Workspaces put one project’s sessions across hosts in one grid, an attention cue marks the session waiting on a yes, and the yes arrives from wherever you are. The [sessions guide](/codex-cli-sessions) goes deeper on the run outliving the terminal, and the [security page](/security) sets out what the server can and cannot see.",
        ],
      },
    ],
    spokes: [
      ...
      },
      {
        ...
        blurb: "An agent and a pair programmer, compared by a party that runs both.",
