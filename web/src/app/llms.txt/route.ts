...
spawnd runs coding agents (Claude Code, Codex, OpenCode, Aider, or any CLI) as persistent terminal sessions on machines you own. One daemon per host dials out, so nothing listens and no ports open. Any browser — a phone's included, installed as a web app — becomes the console: workspaces per project, a grid of live sessions across hosts, attention cues when an agent waits on a yes. Your browser talks to each daemon peer-to-peer, end-to-end encrypted; when a relay is unavoidable it forwards ciphertext it cannot decrypt. Agents authenticate on the host as they always do; spawnd holds no provider credentials. Open source, MIT/Apache-2.0. Daemon for macOS and Linux.
...
- [Guides hub](${SITE}/guides): agent guides, device guides, definitions, fixes
- [Claude Code](${SITE}/claude-code): plans, settings, commands, remote use
- [Codex CLI](${SITE}/codex): the complete guide
- [Run agents in parallel](${SITE}/run-agents-in-parallel): the flagship
- [Coding agents on your phone](${SITE}/coding-agents-on-your-phone): the device row
- [Compared](${SITE}/vs): spawnd against every tool that shares a job with it
