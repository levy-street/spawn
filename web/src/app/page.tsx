...
export default function LandingPage() {
  ...
  // Unknown keeps the historical macOS handoff on the server render. Once the
  // browser is known, only a native desktop OS requests a local fallback.
  ...
  const requestedDownloadPlatform: DesktopPlatform | null = windowsChosen
    ? WINDOWS_DESKTOP_PLATFORM
    : installTarget === "unix" && (detectedOS === "macos" || detectedOS === "unknown")
      ? "darwin-aarch64"
      : null;
  const releaseBuildUrl =
    requestedDownloadPlatform && desktopRelease?.platforms.includes(requestedDownloadPlatform)
      ? desktopDownloadUrl(origin, desktopRelease.version, requestedDownloadPlatform)
      : null;
