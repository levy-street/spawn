//! DataChannel latency bench: like rtc_probe, but echoes every message for
//! up to two minutes so a browser peer can measure round-trip distributions
//! against a bare webrtc-rs stack (no spawnd, no worker, no PTY). Loopback
//! host candidates only. Signaling over stdio as in rtc_probe.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match tokio::time::timeout(Duration::from_secs(30), done_rx.recv()).await {
        Ok(_) => {
            eprintln!("EVENT first-echo");
            // Keep echoing while the browser runs its measurement.
            tokio::time::sleep(Duration::from_secs(120)).await;
            eprintln!("EVENT bench-window-closed");
            Ok(())
        }
        Err(_) => {
