...
fn validate_handle_acl(handle: HANDLE, directory: bool) -> io::Result<()> {
    ...
    // SAFETY: all outputs are valid pointers; the returned descriptor owns the
    // owner and DACL pointers and is freed exactly once below.
    let status = unsafe {
        GetSecurityInfo(
            handle,
            SE_FILE_OBJECT,
    ...
    // SAFETY: descriptor is live and the out pointers are valid.
