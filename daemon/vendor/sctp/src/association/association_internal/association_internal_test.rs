...
/// Regression for webrtc-rs/webrtc#822 and rtc#201. The first fragment fills
/// the entire receive window without becoming readable. The ending fragment is
/// nevertheless the next cumulative TSN and must be admitted so the
/// application can drain the completed message and reopen the window.
#[tokio::test]
async fn test_full_receive_buffer_admits_next_in_sequence_fragment() -> Result<()> {
    let mut a = create_association_internal(Config {
        net_conn: Arc::new(DumbConn {}),
        max_receive_buffer_size: 3,
        max_message_size: 0,
        mtu: 0,
        name: "client".to_owned(),
        local_port: 5000,
    ...
    let stream = a.create_stream(1, false).expect("stream admitted");
    ...
    .await?;
    assert_eq!(a.get_my_receiver_window_credit().await, 0);
    assert!(!stream.reassembly_queue.lock().await.is_readable());

    let final_tsn = a.peer_last_tsn.wrapping_add(1);
    a.handle_data(&ChunkPayloadData {
    ...
    assert_eq!(a.peer_last_tsn, final_tsn);
    assert!(stream.reassembly_queue.lock().await.is_readable());
    Ok(())
}
