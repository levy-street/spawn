...
impl AssociationInternal {
    /// Whether the application can already drain any complete message.
    ///
    /// SPAWN D carries this narrow backport from
    /// https://github.com/webrtc-rs/webrtc/issues/822 and
    /// https://github.com/webrtc-rs/rtc/pull/201. When incomplete fragments
    /// fill the receive window, the next in-sequence chunk may be the only
    /// thing that can complete reassembly and reopen that window. Remove this
    /// helper and the matching admission branch once a published
    /// `webrtc-sctp` release contains the upstream fix.
    async fn has_readable_data(&self) -> bool {
        for stream in self.streams.values() {
            if stream.reassembly_queue.lock().await.is_readable() {
                return true;
            }
        }
        false
            ...
            if let Some(_s) = self.get_or_create_stream(d.stream_identifier) {
                if self.get_my_receiver_window_credit().await > 0 {
                    ...
                    if let Some(last_tsn) = self.payload_queue.get_last_tsn_received() {
                        ...
                        && !self.has_readable_data().await
                    {
                        // Backport of webrtc-rs/rtc#201 for webrtc-rs/webrtc#822:
                        // this chunk can complete the fragments that pinned the
                        // full receive window, so repeatedly dropping it would
                        // deadlock SCTP until the browser sends ABORT.
                        log::debug!(
