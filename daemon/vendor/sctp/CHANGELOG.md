...
* Closing the association closes its pending queue: a writer waiting for room that only a peer's acknowledgement can return gets `ErrStreamClosed` instead of waiting forever, and no longer holds the writer lock while it waits — which kept a stream shutdown, and with it the whole `RTCPeerConnection::close`, from ever running against a peer that had gone silent. `PendingQueue::push`/`append` are fallible.
...
* `association::Config` gained a `mtu` field; struct-literal constructors need `mtu: 0` for the previous behavior
