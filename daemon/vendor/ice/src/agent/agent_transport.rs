...
impl Conn for AgentConn {
    ...
    async fn send(&self, buf: &[u8]) -> std::result::Result<usize, util::Error> {
        ...
        match result {
            // Candidate replacement can race a write on the previous route.
            // Match the typed UDP error here, before wrapping erases its kind.
            // As with an absent candidate above, report a dropped datagram and
            // let SCTP retransmit. Propagating this temporary route failure
            // instead tears down the entire otherwise recoverable association.
            Err(Error::Util(util::Error::Io(error)))
                if matches!(
                    error.0.kind(),
            ...
            {
                log::debug!("dropping ICE datagram on an unavailable route: {error}");
                Ok(0)
            }
            Ok(n) => {
