...
/**
 * Read the v1 refusal discriminator, with a narrow compatibility bridge for
...
export function deviceRegistrationRefusalCode(
  ...
  if (error.status === 409) {
    ...
    if (detail.includes("different root designation") || detail.includes("root designation")) {
      return "root_designation_mismatch";
    }
...
export async function ensureDeviceRegistered(input: {
  ...
  try {
    ...
    // A tombstone is final for the old key and safe to heal without asking:
    // delete only this account's phone identity, mint a new key, and let the
    // ordinary approval machinery take over from the new roster row.
...
export function describeDeviceRegistrationFailure(error: unknown): DeviceRegistrationFailure {
  ...
  if (error instanceof ApiError) {
    ...
    return {
      ...
      reason: "The server could not register this phone.",
      remedy: null,
