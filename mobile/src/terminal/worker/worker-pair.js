  ...
  class ViewChannel extends EventTarget {
    ...
    receive(message) {
      ...
      if (message.event === "open") {
        ...
        try {
          this.fire("open");
          while (this.readyState === "open" && this.earlyReceive.length) {
            const next = this.earlyReceive.shift();
            this.earlyReceiveBytes -= sizeOf(next.data ?? "");
            this.deliver(next);
          }
        } finally {
          this.drainingEarlyReceive = false;
        ...
        // A native ready frame can precede the channel's open event. Deliver
        // it after open so bootstrap and receive-credit acknowledgements have
        // the same ordering on every platform.
        if (this.readyState === "connecting" || this.drainingEarlyReceive) {
          const bytes = sizeOf(message.data ?? "");
          if (this.earlyReceiveBytes + bytes > MAX_RECEIVE || this.earlyReceive.length >= 1024) {
            this.close();
            return;
          }
          this.earlyReceive.push(message);
          this.earlyReceiveBytes += bytes;
