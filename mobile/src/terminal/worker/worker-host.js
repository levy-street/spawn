...
(() => {
  "use strict";
  const root = globalThis.spawnWorker;
  function createHostProtocol(api) {
    ...
    async function waitForWritable() {
      ...
      await new Promise((resolve, reject) => {
        ...
        const onClose = () => {
          cleanup();
          reject(new Error("Host-control channel closed."));
        };
        channel.addEventListener("bufferedamountlow", onWritable);
    ...
    async function sendHostMessage(message) {
      try {
        if (message.type === "host-cancel") {
          await sendFrame({ version: 1, type: "cancel", request_id: message.requestId });
          return;
        }
  ...
  root.createHostProtocol = createHostProtocol;
  createHostProtocol(root);
})();
