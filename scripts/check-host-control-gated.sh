...
# resolver or reuse the authenticated device connection, so the gate's invariant holds without exception: a pinned host is
...
while IFS= read -r line; do
  ...
  if ! sed -n "${lineno},$((lineno + 12))p" "$file" | grep -Eq 'resolveSignedRtcTrust|sharedConnection'; then
...
if [[ $status -ne 0 ]]; then
  printf 'each construction must pass resolveSignedRtcTrust or a sharedConnection whose owner supplies it (see signed-rtc-trust.ts)\n' >&2
  exit 1
fi
